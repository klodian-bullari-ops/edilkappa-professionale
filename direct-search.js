(function () {
  ...
  function buildSearchResults() {
    ...
    if (isOffice()) {
      ...
      add(db.sites, { type: 'Cantiere', icon: '🏗️', action: 'site', title: (item) => item.title, meta: (item) => `${item.client || ''} · ${item.address || ''}`, mediaCount: (item) => (db.reports || []).filter((report) => String(report.site || report.siteId) === String(item.id)).reduce((total, report) => total + Math.max(Number(report.photoCount || 0), Array.isArray(report.photos) ? report.photos.length : 0), 0) });
      ...
      add((db.sites || []).filter((item) => item.worker === teamId), { type: 'Cantiere', icon: '🏗️', action: 'site', title: (item) => item.title, meta: (item) => `${item.client || ''} · ${item.address || ''}`, mediaCount: (item) => (db.reports || []).filter((report) => String(report.site || report.siteId) === String(item.id)).reduce((total, report) => total + Math.max(Number(report.photoCount || 0), Array.isArray(report.photos) ? report.photos.length : 0), 0) });
