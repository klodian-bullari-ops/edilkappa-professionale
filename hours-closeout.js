(function () {
  ...
  worker = function () {
    ...
    (database().roofs || []).filter((item) => item.worker === teamId && item.status !== 'Completato').forEach((item) => tasks.push({ icon: '🧹', title: item.type, client: item.client, address: item.address, status: item.status, action: `updateRoofTask('${item.id}')`, label: 'Aggiorna e carica foto' }));
    (database().drains || []).filter((item) => item.worker === teamId && item.status !== 'Completato').forEach((item) => tasks.push({ icon: '🕳️', title: item.type, client: item.client, address: item.address, status: item.status, action: `updateDrainTask('${item.id}')`, label: 'Aggiorna e carica foto' }));
    const missingCount = missingCompletedSites(person).length;
    return `<div class="workerHero"><h2>Ciao, ${esc(roleName())}</h2><p>${esc(team?.name || 'Senza squadra')} · qui trovi gli incarichi assegnati alla tua squadra.</p></div>${workerAlertHtml(person)}<div class="grid quick"><button onclick="go('hours')"><span>⏱️</span>Comunica le mie ore${missingCount ? ` (${missingCount})` : ''}</button><button onclick="go('report')"><span>📝</span>Nuovo rapportino</button><button onclick="callOwner()"><span>☎️</span>Chiama titolare</button></div>${pageHead('Lavori della mia squadra', `${tasks.length} incarichi`)}<div class="list">${tasks.map((item) => `<div class="card ${item.closeout ? 'hoursCloseoutCard' : ''}"><div class="row" style="border:0;padding:0"><div class="rowIcon">${item.icon}</div><div class="rowBody"><b>${esc(item.title)}</b><small>${esc(item.client)} · ${esc(item.address)}</small></div>${item.closeout ? '<span class="pill red">Ore mancanti</span>' : badge(item.status)}</div><div class="actions" style="margin-top:14px"><button class="btn ${item.closeout ? 'red' : 'green'}" onclick="${item.action}">${item.label}</button><a class="btn light" href="https://maps.apple.com/?q=${encodeURIComponent(item.address || '')}">Indicazioni</a></div></div>`).join('') || '<div class="empty">La tua squadra non ha incarichi assegnati.</div>'}</div>`;
  };
