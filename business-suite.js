(function () {
  ...
  openPdfUpload = function () {
    ...
    modal('Carica preventivo PDF', `<div class="formGrid"><div class="field full"><label>File PDF</label><input name="pdf" type="file" accept="application/pdf,.pdf" required><small>Il documento viene conservato nell’archivio cloud protetto.</small></div>${field('Numero preventivo', 'code', 'text', `PREV-${new Date().getFullYear()}-${String(db.quotes.length + 1).padStart(3, '0')}`)}<div class="field"><label>Cliente</label><select name="client" onchange="refreshInterventionSelect(this.form)">${clientOptions(context.client || '')}</select></div><div class="field"><label>Intervento</label><select name="interventionId">${window.interventionOptions ? window.interventionOptions(context.client || '', context.interventionId || '') : '<option value="">Da assegnare</option>'}</select></div>${field('Oggetto', 'subject', 'text', 'Preventivo caricato', true)}${field('Importo netto € (facoltativo)', 'net', 'number', '0')}${field('Data', 'date', 'date', localToday())}<div class="field"><label>Stato</label><select name="status"><option>Bozza</option><option>Inviato</option><option>In attesa</option><option>Accettato</option><option>Rifiutato</option></select></div>${mediaInput()}</div>`, async (formData) => {
      ...
      if (window.EdilKappaCloud?.ready && window.EdilKappaCloud.uploadDocument) stored = await window.EdilKappaCloud.uploadDocument(file, { documentId: id, category: 'Preventivo', client, interventionId });
      ...
      db.quotes.push({ id, ...stored, code: formData.get('code'), client, clientId: db.condomini.find((entry) => entry.name === client)?.id || '', interventionId, subject: formData.get('subject'), net: Number(formData.get('net') || 0), date: formData.get('date'), status: formData.get('status'), media });
    });
    setTimeout(() => window.clearArchiveContext?.(), 0);
  };

  openDrone = function (id) {
    const draft = db.drone.find((entry) => entry.id === id) || { id: uid('d'), client: '', interventionId: '', date: localToday(), area: 'Copertura e facciate', findings: '', status: 'Da relazionare', media: [] };
    const item = window.prepareArchiveItem ? window.prepareArchiveItem(draft, id) : draft;
    modal(id ? 'Modifica videoispezione' : 'Nuova videoispezione', `<div class="formGrid"><div class="field"><label>Cliente</label><select name="client" onchange="refreshInterventionSelect(this.form)">${clientOptions(item.client)}</select></div><div class="field"><label>Intervento</label><select name="interventionId">${window.interventionOptions ? window.interventionOptions(item.client, item.interventionId) : '<option value="">Da assegnare</option>'}</select></div>${field('Data', 'date', 'date', item.date)}${field('Area ispezionata', 'area', 'text', item.area, true)}<div class="field full"><label>Anomalie rilevate</label><textarea name="findings" required>${esc(item.findings)}</textarea></div><div class="field"><label>Stato</label><select name="status">${selectOptions(['Da relazionare', 'Relazione pronta', 'Completato'], item.status)}</select></div>${mediaInput()}${mediaFiles(item).length ? `<div class="field full"><span class="mediaCount">📎 ${mediaFiles(item).length} file già allegati</span></div>` : ''}</div>`, async (formData) => {
      const client = formData.get('client');
      const addedMedia = await uploadMediaFiles(formData.getAll('media'), { ownerId: item.id, category: 'Videoispezione drone', client });
      const data = { client, clientId: db.condomini.find((entry) => entry.name === client)?.id || item.clientId || '', interventionId: formData.get('interventionId'), date: formData.get('date'), area: formData.get('area'), findings: formData.get('findings'), status: formData.get('status'), media: mediaFiles(item).concat(addedMedia), updatedAt: new Date().toISOString() };
