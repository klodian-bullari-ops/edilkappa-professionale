(function () {
  ...
  function localSnapshot() {
    ...
    const leads = db.leads || [], sites = db.sites || [], quotes = db.quotes || [], reports = db.reports || [], timesheets = db.timesheets || [], absences = db.absences || [], payments = db.payments || [], deadlines = db.deadlines || [];
  ...
  function viewHtml() {
    ...
    return `<div class="opsHero"><div><h2>Centro operativo EdilKappa</h2><p>I sei agenti controllano richieste, cantieri, preventivi, amministrazione, guadagni e notifiche. Ogni azione resta sotto il tuo controllo.</p></div><div class="actions"><button class="btn lime" onclick="edilkappaOperationsRefresh()" ${state.refreshing ? 'disabled' : ''}>${state.refreshing ? 'Analisi in corso…' : 'Aggiorna analisi AI'}</button></div></div><div style="height:15px"></div>
      <div class="grid stats">${stat('Urgente oggi', m.urgent || 0, '⚠')}${stat('Richieste nuove', m.newRequests || 0, '📥')}${stat('Cantieri fermi', m.stalledSites || 0, '🏗️')}${stat('Foto oggi', m.photosToday || 0, '📷')}${stat('Ore mancanti', m.missingHours || 0, '⏱️')}${stat('Pagamenti scaduti', m.overduePayments || 0, '€')}</div>
