(function () {
  ...
  window.completeInspection = function (id) {
    ...
    modal('Sopralluogo eseguito', `<div class="notice"><b>${esc(item.client)}</b><br>${esc(item.address || '')}<br>Intervento: ${esc(intervention.title)}</div><div style="height:14px"></div><div class="formGrid">
      <div class="field full"><label for="inspectionOutcome">Esito del sopralluogo</label><textarea id="inspectionOutcome" name="outcome" required placeholder="Che cosa hai verificato e qual è la causa del problema?">${esc(item.outcome || '')}</textarea></div>
      <div class="field full"><label for="inspectionMeasurements">Misure rilevate</label><textarea id="inspectionMeasurements" name="measurements" placeholder="Metri, superfici, spessori, quantità e altre misure">${esc(item.measurements || '')}</textarea></div>
      <div class="field full"><label for="inspectionRecommendations">Lavorazioni consigliate</label><textarea id="inspectionRecommendations" name="recommendations" required placeholder="Come consigli di risolvere il problema?">${esc(item.recommendations || '')}</textarea></div>
      <div class="field full"><label for="inspectionTechnicalNotes">Note tecniche</label><textarea id="inspectionTechnicalNotes" name="technicalNotes" placeholder="Accesso, sicurezza, materiali, difficoltà o richieste del cliente">${esc(item.technicalNotes || '')}</textarea></div>
      ...
      <div class="field full"><label for="inspectionMediaFiles">Scegli foto e video</label><input id="inspectionMediaFiles" name="media" type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif,.jpg,.jpeg,.png,.webp,.heic,.heif,video/mp4,video/quicktime,video/webm,.mp4,.mov,.webm" multiple aria-describedby="inspectionMediaStatus" onchange="rememberInspectionMedia('${esc(item.id)}',this)"><small id="inspectionMediaStatus">${mediaCount ? `${mediaCount} file già salvati. ` : ''}HEIC e HEIF vengono riconosciuti automaticamente. Dal telefono puoi usare Fotocamera, Libreria foto o Sfoglia.</small></div>
    </div>`, async (formData) => {
