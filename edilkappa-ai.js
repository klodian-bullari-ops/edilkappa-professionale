(function () {
  ...
  css.textContent = `
    ...
    .ekAiDocumentFlow{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:7px;margin:0 0 13px}.ekAiDocumentStep{padding:8px 10px;border:1px solid #dce4df;border-radius:11px;background:#f5f7f6;color:#718078;font-size:11px;font-weight:850}.ekAiDocumentStep b{display:block;margin-bottom:2px;color:inherit}.ekAiDocumentStep.done{background:#e8f5ec;border-color:#b9dcc5;color:#176542}.ekAiDocumentStep.current{background:#fff7cc;border-color:#e4c94f;color:#6b5500}
  ...
  function documentFlowHtml(messages) {
    if (state.mode !== "work") return "";
    const artifactMessage = messages.slice().reverse().find((message) => message?.artifact);
    const artifact = artifactMessage?.artifact;
    const saved = artifact ? artifactSavedItem(artifact) : null;
    const values = [state.attachments.length > 0 || messages.some((message) => message.role === "user"), Boolean(artifact), Boolean(artifact), Boolean(saved)];
    const labels = [["1. Materiale","Foto, misure e richiesta"],["2. Anteprima","Documento leggibile"],["3. Verifica","Prezzi e controlli"],["4. Salvataggio","Intervento e cliente"]];
    const current = Math.max(0, values.findIndex((value) => !value));
    return `<div class="ekAiDocumentFlow">${labels.map(([title,detail],index) => `<div class="ekAiDocumentStep ${values[index] ? "done" : index === current ? "current" : ""}"><b>${title}</b>${detail}</div>`).join("")}</div>`;
  }
