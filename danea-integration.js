(function () {
  ...
  function outlookPanel() {
    ...
    if (status?.health === 'active') return `<section class="daneaOutlook"><div class="daneaOutlookHead"><div><b>✓ Importazione Danea automatica attiva</b><small style="display:block">Outlook → ${esc(status.mailbox || 'info@edilkappa.com')} → EdilKappa · ultimo controllo ${esc(dateTimeText(new Date(status.lastPollAtMs).toISOString()))}${status.lastReceivedAtMs ? ` · ultima richiesta ${esc(dateTimeText(new Date(status.lastReceivedAtMs).toISOString()))}` : ' · in attesa della prima richiesta'}</small></div></div>${status.lastError ? `<div class="notice">Ultimo errore di importazione: ${esc(status.lastError)}</div>` : ''}</section>`;
    if (status?.health === 'stale') return `<section class="daneaOutlook"><div class="daneaOutlookHead"><div><b>⚠ Controllo Gmail Danea non recente</b><small style="display:block">Il gestionale risponde, ma non riceve un heartbeat da oltre 20 minuti.${status.lastPollAtMs ? ` Ultimo controllo: ${esc(dateTimeText(new Date(status.lastPollAtMs).toISOString()))}.` : ''}</small></div><button class="btn sm light" onclick="retryDaneaBridgeStatus()">Ricontrolla</button></div><div class="notice">Apri Apps Script ed esegui di nuovo <b>installDaneaTrigger()</b>. Le richieste già archiviate restano disponibili.</div></section>`;
    if (status?.health === 'error') return `<section class="daneaOutlook"><div class="daneaOutlookHead"><div><b>⚠ Il controllo Gmail Danea segnala un errore</b><small style="display:block">${esc(status.lastPollError || status.lastError || 'Controlla Apps Script e le autorizzazioni Gmail.')}</small></div><button class="btn sm light" onclick="retryDaneaBridgeStatus()">Ricontrolla</button></div></section>`;
    ...
    return `<section class="daneaOutlook"><div class="daneaOutlookHead"><div><b>Configurazione importazione Danea da completare</b><small style="display:block">Aggiorna Apps Script ed esegui <b>installDaneaTrigger()</b>. Lo stato diventerà verde al primo controllo Gmail riuscito, anche senza nuove richieste.</small></div></div></section>`;
  }
  ...
  window.daneaRequestsView = function () {
    ...
    return pageHead('Richieste Danea', 'Incarichi ricevuti dagli amministratori e gestione interna EdilKappa', '<button class="btn light" onclick="openDaneaImport()">↓ Importa testo/e-mail</button><button class="btn lime" onclick="openDaneaRequest()">＋ Nuova manuale</button>') + outlookPanel() +
