...
function loadModule() {
  const context: Record<string, unknown> = {
    ...
    db: {
      ...
      timesheets: [
        { id: 'h-1', date: '2026-08-04', worker: 'worker-1', workerName: 'Ajet', team: 'team-1', teamName: 'Squadra A', siteId: 'site-1', job: 'Rifacimento tetto · Condominio Alfa', hours: 8 },
        { id: 'h-2', date: '2026-08-04', worker: 'worker-1', workerName: 'Ajet', team: 'team-1', teamName: 'Squadra A', siteId: 'site-1', job: 'Rifacimento tetto · Condominio Alfa', hours: 6 },
        { id: 'h-3', date: '2026-08-04', worker: 'worker-2', workerName: 'Ciccio', team: 'team-2', teamName: 'Squadra B', job: 'Rifacimento tetto · Condominio Alfa', hours: 7 },
        { id: 'h-4', date: '2026-08-05', worker: 'worker-2', workerName: 'Ciccio', team: 'team-2', teamName: 'Squadra B', workType: 'outside', job: 'Magazzino / preparazione', hours: 2 },
        { id: 'h-5', date: '2026-08-05', worker: 'worker-3', workerName: 'Qemal', team: 'team-2', teamName: 'Squadra B', job: 'Cantiere da chiarire', hours: 3 }
      ],
      edilconnect: [
        { id: 'edilconnect-site-site-1', recordType: 'site', siteId: 'site-1', commissionType: 'Privato', companyRole: 'Affidataria', totalWorkValue: 80000, buildingWorkValue: 70000, subjectMode: 'Automatico', cuc: 'CNCEC1234567890', dnlStatus: 'Comunicata', certificateStatus: 'Da richiedere a fine lavori', status: 'Attivo' }
      ],
      reports: []
    },
...
test('il CUC segue il formato ufficiale e le attività esistenti restano selezionabili', () => {
  const api = loadModule();
  assert.equal(api.validCuc('CNCEC1234567890'), true);
...
test('le ore esistenti vengono collegate al cantiere senza reinserimento', () => {
  ...
  assert.equal(data.matrix.filter((row) => row.workerName === 'Ajet')[0]?.hours, 14);
});
...
test('il CSV usa UTF-8, separatore Excel italiano e include il CUC', () => {
  ...
  assert.match(csv, /CNCEC1234567890/);
...
test('la nuova funzione è disponibile offline', () => {
  assert.match(serviceWorker, /v45-ai-multimediale-preventivi-relazioni/);
  ...
  assert.match(indexHtml, /\.\/sw\.js\?v=45/);
});
